...
const SideBar = ({ userRole }: SideBarProps) => {
    ...
    return (
        ...
        <NavigationMenu.Root className={` h-screen bg-primary duration-300 ${expanded ? 'w-80' : 'w-20'} `}>
            ...
            <AlertDialog.Root>
                <AlertDialog.Trigger asChild className={`absolute bottom-0 left-0 w-full bg-secondary text-alternative-text`}>

                    <button className={`m-auto px-6 gap-24 py-5 justify-between hover:-translate-x-0.5 transition-all ease-in-out duration-300 `}>
                        {expanded ?
                            <Flex gap="4" align="center" justify="start">
                                <Card variant="ghost" className={`${!expanded && "scale-0"} duration-300`} >
                                    <Flex align="center">
                                        <Flex direction="column" justify="end">
                                            <Text as="label" weight="bold" className="text-[15px]">
                                                {getFirstAndLastName(`${userData?.fullName}`)}
                                            </Text>
                                            <Text as="label" weight="light" className="text-[10px] italic" >
                                                {userRole}
                                            </Text>
                                        </Flex>
                                    </Flex>
                                </Card>
                                <Icon.SignOut size={35} />
                            </Flex>
                            :
                            <Icon.SignOut size={35} className="" />}
                    </button>
